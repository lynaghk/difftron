...
(transient-define-prefix
 ...
 [["Rust Dive" ("g" "Refresh" rust-dive-magit-refresh)
   ("f"
    "Cycle grouping"
...
working tree rooted at the current repository."
  (interactive (let*
                   ((repo-root (rust-dive-magit--repo-root))
                    ...
                    (lhs
                     (read-string "Left snapshot/rev: " default-lhs))
                    (rhs
                     (read-string "Right snapshot/rev: " default-rhs))
                    (path-input
                     (read-string
                      "Filter paths (comma-separated, optional): ")))
                 (list
                  lhs rhs (rust-dive-magit--split-paths path-input))))
  (let* ((default-directory (rust-dive-magit--repo-root))
         ...
         (payload
          (rust-dive-magit--run-command default-directory args)))
...
(defun rust-dive-magit-refresh ()
  ...
  (interactive)
  (unless (and rust-dive-magit--command-args
...
(defun rust-dive-magit--insert-kind-groups (items)
  ...
  (let ((groups (rust-dive-magit--group-items-by-kind items)))
    (if groups
        (dolist (group groups)
          (rust-dive-magit--insert-kind-group
           (car group) (cdr group)))
      (insert "No entities\n"))))
  ...
  (magit-insert-section
   ...
   (magit-insert-heading
    (propertize (format "%s%s (%d)"
                        (rust-dive-magit--indent
                         depth)
                        file (length items))
                'font-lock-face
                (rust-dive-magit--heading-face depth)))
   (dolist (item items)
  ...
  (let ((entity (plist-get item :entity)))
    (magit-insert-section
     ...
     (add-text-properties
      ...
      '(mouse-face
        highlight help-echo "RET visits source, TAB toggles section"))
...
(defun rust-dive-magit--insert-structured-diff-row (row column-width)
  ...
  (rust-dive-magit--insert-structured-side
   (plist-get row :left) 'left column-width)
  (insert " | ")
  (rust-dive-magit--insert-structured-side
   (plist-get row :right) 'right column-width)
  (insert "\n"))
  ...
  (if side
      (let ((start (point)))
        (rust-dive-magit--insert-structured-segments
         (plist-get side :segments) side-name column-width)
        (when (eq side-name 'left)
          (insert
           (make-string
            (max 0 (- column-width (- (point) start))) ?\s))))
    (when (eq side-name 'left)
      ...
      (when (> remaining 0)
        (let* ((text
                (truncate-string-to-width
                 (or (plist-get segment :text) "") remaining 0 nil))
               (face
                (rust-dive-magit--segment-face
                 side-name (plist-get segment :kind))))
...
(defun rust-dive-magit--run-command (repo-default-directory args)
  ...
  (with-temp-buffer
    (let ((default-directory repo-default-directory)
...
(defun rust-dive-magit-register-magit-diff-suffix ()
  ...
  (when (and (featurep 'magit-diff)
    ...
    (transient-append-suffix
     'magit-diff "d" rust-dive-magit--magit-diff-suffix)))
...
When FILE is non-nil, it is the path passed by `after-load-functions'."
  (when (and rust-dive-magit-bindings-mode
             (or (null file)
                 (string= (file-name-base file) "magit-diff")))
    (remove-hook
