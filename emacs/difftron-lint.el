;;; difftron-lint.el --- Formatting and lint helpers for difftron -*- lexical-binding: t; -*-
...
(defconst difftron-lint--private-symbol-allowlist nil
  "Private symbols intentionally exempt from unused-symbol checks.")

(defun difftron-lint--repo-root ()
...
(defun difftron-lint--owned-elisp-files ()
  "Return repo-owned Emacs Lisp files covered by formatting and linting."
  (append
   (directory-files
    (expand-file-name "emacs" (difftron-lint--repo-root))
    t
    "\\.el\\'")
   (directory-files
    (expand-file-name "emacs/test-config"
		      (difftron-lint--repo-root))
    t "\\.el\\'")))

(defun difftron-lint--checkdoc-files (files)
...
(defun difftron-lint--private-definition-symbol (form)
  ...
  (when
    ...
    (let ((symbol (nth 1 form)))
      (when
          (and (symbolp symbol)
               (string-prefix-p "difftron--" (symbol-name symbol)))
        symbol))))

(defun difftron-lint--form-references-symbol-p (form symbol)
  ...
  (cond
   ...
   ((consp form)
    (or
     (difftron-lint--form-references-symbol-p
      (car form)
      symbol)
     (difftron-lint--form-references-symbol-p
      (cdr form)
...
(defun difftron-lint--unused-private-definitions
    (forms allowlist)
  ...
  (let*
      (
       (definitions
        (delq
         nil
         (mapcar
          #'difftron-lint--private-definition-symbol
          forms)))
    ...
    (dolist (form forms)
      (when-let
          (
           (symbol
            (difftron-lint--private-definition-symbol form)))
...
(defun difftron-lint--collect-unused-private-definitions
    (files)
  ...
  (let (results)
    (dolist (file files)
      (let*
          (
           (forms (difftron-lint--read-forms file))
           (unused
            (difftron-lint--unused-private-definitions
             forms
             difftron-lint--private-symbol-allowlist)))
        (when unused
          (push (cons file unused) results))))
    (nreverse results)))

(defun difftron-lint--formatted-contents (file)
...
(defun difftron-lint--check-formatting (files)
          ...
          (
           (current
            (with-temp-buffer
              (insert-file-contents file)
              (buffer-string)))
           (formatted (difftron-lint--formatted-contents file)))
...
(defun difftron-lint--apply-formatting (files)
  ...
  (dolist (file files)
	...
	(
      ...
      (unless (string= current formatted)
        (with-temp-file file
          (insert formatted))
        (difftron-lint--message "Formatted %s"
				(file-relative-name file))))))

(defun difftron-lint--byte-compile-file (file)
...
(defun difftron-lint--run-byte-compile (files)
  "Byte-compile FILES with warnings treated as errors."
  (dolist (file files)
    (difftron-lint--message "Byte-compiling %s"
			    (file-relative-name file))
    (difftron-lint--byte-compile-file file)))

(defun difftron-lint--run-checkdoc (files)
  "Run `checkdoc' against FILES."
  (require 'checkdoc)
  (dolist (file files)
    (difftron-lint--message "Running checkdoc on %s"
			    (file-relative-name file))
    (with-temp-buffer
      (insert-file-contents file)
      (delay-mode-hooks
        ...
        (when problems
          (dolist (problem (nreverse problems))
            (pcase-let ((`(,text ,start ,_end) problem))
              (save-excursion
                (goto-char start)
                (difftron-lint--message "%s:%s: checkdoc: %s"
					(file-relative-name file)
					(line-number-at-pos)
					text))))
          (error
...
(defun difftron-lint--run-package-lint (file)
  "Run `package-lint' against FILE."
  (require 'package-lint)
  (difftron-lint--message "Running package-lint on %s"
			  (file-relative-name file))
  (with-temp-buffer
    (insert-file-contents file)
    (delay-mode-hooks
      ...
      (when issues
        (dolist (issue issues)
          (pcase-let ((`(,line ,column ,type ,message) issue))
            (difftron-lint--message "%s:%s:%s: %s: %s"
				    (file-relative-name file)
				    line
				    column
				    type
...
(defun difftron-lint--run-relint (files)
  "Run `relint' against FILES."
  (require 'relint)
  (dolist (file files)
    (difftron-lint--message "Running relint on %s"
			    (file-relative-name file))
...
(defun difftron-lint--run-unused-private-check (files)
  "Fail when FILES contain unused private definitions."
  (let
      (
       (unused
        (difftron-lint--collect-unused-private-definitions
         files)))
    (when unused
      (dolist (entry unused)
        (difftron-lint--message
...
(defun difftron-format-batch-and-exit ()
  "Format repo-owned Emacs Lisp files and exit."
  (interactive)
  (condition-case err
      (progn
	(difftron-lint--apply-formatting
         (difftron-lint--owned-elisp-files))
	(kill-emacs 0))
    (error
     (difftron-lint--message "%s" (error-message-string err))
     (kill-emacs 1))))

(defun difftron-lint-batch-and-exit ()
  "Check formatting and run lint for repo-owned Emacs Lisp files."
  (interactive)
  (condition-case err
      (let*
	  (
           (owned-files (difftron-lint--owned-elisp-files))
           (package-file
            (expand-file-name "emacs/difftron.el"
			      (difftron-lint--repo-root))))
	(difftron-lint--message "Checking formatting")
	(difftron-lint--check-formatting owned-files)
	(difftron-lint--run-byte-compile owned-files)
	(difftron-lint--run-checkdoc
         (difftron-lint--checkdoc-files owned-files))
	(difftron-lint--run-package-lint package-file)
	(difftron-lint--run-relint owned-files)
	(difftron-lint--run-unused-private-check owned-files)
	(kill-emacs 0))
    (error
     (difftron-lint--message "%s" (error-message-string err))
     (kill-emacs 1))))

(provide 'difftron-lint)

;;; difftron-lint.el ends here
